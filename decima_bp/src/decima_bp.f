...
c       parameter(nnodem=1000)
      ...
      character(len = 100) :: file_input, file_node_index, 
     :  file_expt_index, file_prio, file_data, file_pert
      integer unit_id
      ...
      ! program discription
      write(*, *) "BP-based algorithm for network inference " //
     :            "from perturbation data"
      write(*, *) "Authors: Anil Korkut @ MDACC, Chris Sander @ Harvard"
      write(*, *) ""
      ...
      if (num_args == 0) then
        write(*, *) "DESCRIPTION"
        write(*, *) "-i input", char(9), "Run your inference"
      ...
      do while (.true.)
        ...
        if (args(n) == "-i") then
          ...
          write(*, *) "n_expts", char(9), "89"
          write(*, *) "n_nodes", char(9), "99"
          write(*, *) "n_wvals", char(9), "11"
          write(*, *) "wmax", char(9), "1.00E+00"
          write(*, *) "threshold", char(9), "1.00E-06"
          write(*, *) "lambda", char(9), "5.00"
          write(*, *) "beta", char(9), "2.00"
          write(*, *) "n_priors", char(9), "154"
          write(*, *) "n_models", char(9), "80"
          write(*, *) "file_node_index", char(9), "node_index.txt"
          write(*, *) "file_expt_index", char(9), "expt_index.txt"
          write(*, *) "file_prio", char(9), "prio.txt"
          write(*, *) "file_data", char(9), "data.txt"
          write(*, *) "file_pert", char(9), "pert.txt"
          write(*, *) ""
          stop
        end if
        n = n + 1
     ...
     :                " something was wrong"
        ...
        if (line(1:1) == "#" .or. line(1:1) == "") then
          ...
          if (buf_str(1) == "n_expts") then
            read(buf_str(2), *) NEXPTS
          else if (buf_str(1) == "n_nodes") then
            read(buf_str(2), *) NNODES
          else if (buf_str(1) == "n_wvals") then
            read(buf_str(2), *) Nwvals
          else if (buf_str(1) == "wmax") then
            read(buf_str(2), *) Wijmax
          else if (buf_str(1) == "threshold") then
            read(buf_str(2), *) thresh
          else if (buf_str(1) == "lambda") then
            read(buf_str(2), *) lambda
          else if (buf_str(1) == "beta") then
            read(buf_str(2), *) beta
          else if (buf_str(1) == "n_priors") then
            read(buf_str(2), *) n_pri
          else if (buf_str(1) == "n_models") then
            read(buf_str(2), *) n_dec
          else if (buf_str(1) == "file_node_index") then
            read(buf_str(2), *) file_node_index
          else if (buf_str(1) == "file_expt_index") then
            read(buf_str(2), *) file_expt_index
          else if (buf_str(1) == "file_prio") then
            read(buf_str(2), *) file_prio
          else if (buf_str(1) == "file_data") then
            read(buf_str(2), *) file_data
      ...
      write(*, *) "n_expts", char(9), NEXPTS
      write(*, *) "n_nodes", char(9), NNODES
      write(*, *) "n_wvals", char(9), Nwvals
      write(*, *) "wmax", char(9), Wijmax
      write(*, *) "threshold", char(9), thresh
      write(*, *) "lambda", char(9), lambda
      write(*, *) "beta", char(9), beta
      write(*, *) "n_priors", char(9), n_pri
      write(*, *) "n_models", char(9), n_dec
      write(*, *) "file_node_index", char(9), file_node_index
      write(*, *) "file_expt_index", char(9), file_expt_index
      write(*, *) "file_prio", char(9), file_prio
      write(*, *) "file_data", char(9), file_data
      write(*, *) "file_pert", char(9), file_pert
      write(*, *) ""
      
      ! note of results
      write(*, *) "Note: 1) the output of the decimation process " //
     :            "will be written in 'decimation_output.txt'"
      write(*, *) "      2) the infered models will be written " // 
     :            "in files with prefix 'model_'"
      write(*, *) ""
      call flush()
...
ccc  third column: node type 1: proteomic 2: phenotypic 3: activity (drug coupling node)
       open(8,file=file_node_index)
       do i=1,n_vax
       !read(8,*)protnamei(i),in(i),ntypo(i)
         read(8, '(A)', iostat = io_status) line
         if (io_status > 0) then
           write(*, *) "Error: check ", file_node_index, 
     :                 " something was wrong"
           stop
         else if (io_status < 0) then
           exit
         ...
         line = trim(adjustl(line))
         call str_split(line, buf_str)
         read(buf_str(1:3), *) protnamei(i), in(i), ntypo(i)
       enddo
       close(8)
      ...
      do i=1,n_vax 
        if(in(i) == 1)then
          NNODES = NNODES + 1
          if ((ntypo(i) == 1) .or. (ntypo(i) == 2)) then
            n_obs = n_obs + 1
          end if
        end if   
      end do
...
cc 2nd column 1: experiment included in calculations 0: experiment not included
       ...
       expcg=0
       open(11,file=file_expt_index)       
       do i=1,n_exx
       !read(11,"(15x,i1,3x,i1)")inx(i)
         read(11, '(A)', iostat = io_status) line
         if (io_status > 0) then
           write(*, *) "Error: check ", file_expt_index, 
     :                 " something was wrong"
           stop
         else if (io_status < 0) then
           exit
...
ccccccccccccccccccccccccccccccccccccccccccccc	
       ! allocate and initialize prior
       allocate(prior_pri1(n_pri))
       allocate(prior_dist(n_pri))
       allocate(prior_pri2(n_pri))
       allocate(prior_priv(n_pri))
       allocate(prior_support(n_pri))
       prior_pri1(:) = ""
       prior_pri2(:) = ""
       prior_priv(:) = 0
       prior_dist(:) = 0.0
       prior_support(:) = 0.0
       call parse_prior(n_pri, file_prio)
...
c start decimation loop   
     ...
     :,n_pri,thresh,beta,lambda,NEXPTS,wijmax,ntyp,file_prio)
...
!     :(pmto(deci,kkk),kkk=1,NWVALS),Wvals(dmax(deci))
       write(56,"(2(I7,1X),2(A11,1X),1000(F12.8,1X))")
     :round,deci,name_ii(bm(deci),cm(deci)),
     :protname(cm(deci)),Wvals(dmax(deci)),
     :(pmto(deci,kkk),kkk=1,NWVALS)
...
116   if((deci.eq.n_deci) 
     :    .or. (conflag .eq. 0)) !MAY BE INCLOMPLETE SOLUTION
     :  then
...
c	gaussian term to be included       
       subroutine parse_prior(n_pri, file_prio) 
       use decima_bp
       integer :: n_pri
       character*(*) :: file_prio
       character(len = 10000) :: line
       ...
       open(77, file = file_prio)
       do i=1, n_pri
         read(77, '(A)', iostat = io_status) line
         if (io_status > 0) then
           write(*, *) "Error: check ", file_node_index, 
     :                 " something was wrong"
           stop
         ...
         line = trim(adjustl(line))
         call str_split(line, buf_str)
         read(buf_str(1:5), *) prior_pri1(i), prior_dist(i),
     :     prior_pri2(i), prior_priv(i), prior_support(i)
       end do
       close(77)
       end subroutine
...
c       print*,pri1(i),dist(i),pri2(i),priv(i),support(i)
       !enddo     
         pri1(1:n_pri) = prior_pri1(1:n_pri)
         dist(1:n_pri) = prior_dist(1:n_pri)
         pri2(1:n_pri) = prior_pri2(1:n_pri)
         priv(1:n_pri) = prior_priv(1:n_pri)
         support(1:n_pri) = prior_support(1:n_pri)
